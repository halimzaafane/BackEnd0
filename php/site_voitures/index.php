...
if(isset($_GET['marque']) or isset($_GET['kilometrage']) or isset($_GET['couleur'])) {
    $recherche = htmlspecialchars($_GET['marque']);
...
<body>
   ...
   <form method="GET" class="formu">
    <i class="fa fa-car" for="marque"><span> Rechercher par Marque: </span></i>
    <input type="search" name="marque" placeholder=" Marques " autocomplete="off" style="border-radius:10px"><br/><br/>
...
//   ****** HEREDOC ********
    ...
    echo <<<html
                <div class="card alignement" id="rond" style="width: 30rem ;">  
                <img src="{$voitures['src']}" class="card-img-top" id="rond" alt="...">
                <div class="card-body">
                <p> Marque : {$voitures['Marque']}</p>
                <p> couleur : {$voitures['Couleur_originale']}</p>
                <p> kilometrage : {$voitures['kilometrage']} kms</p>
                <a href="pagePhoto.php" class="card-link">voir la voiture</a>
                </div>
                </div>
            html; 

    // cette partie est fonctionnelle sous form de tableau
